...
package com.example.memo.purchase.trade_common.controller.reserve;
...
@Controller
@RequestMapping("/purchase/trade/reserve")
@RequiredArgsConstructor
public class ReservePageViewController {
	...
	public String step1(HttpSession session) {
		...
		if (dto == null || dto.getFlow() == null || dto.getFlow().isBlank()) {
			return "redirect:/purchase/trade/reserve";
		}
		...
		return "purchase/trade/reserve/step1";
	}
	...
	public String step2AfterRisk(HttpSession session) {
		...
		session.setAttribute("ReserveValueDto", dto);
		return "purchase/trade/reserve/step2_reserve"; // 바로 뷰
	}

	/*
	 * 테스트용 함수
	 * 
	 * @GetMapping("/dummy") public String dummy() { return
	 * "purchase/trade/step1_form"; }
	 */
	...
	public String step2Next(@RequestParam(required = false, name = "targetProdIdList") String targetProdIdListJson,
			...
			@RequestParam(required = false, name = "fileUrlList") String fileUrlListJson, HttpSession session) {
		ReserveValueDto dto = (ReserveValueDto) session.getAttribute("ReserveValueDto");
	...
	public String step4Next(ReserveValueDto incoming, HttpSession session) {
		...
		ReserveValueDto dto = (ReserveValueDto) session.getAttribute("ReserveValueDto");
		session.setAttribute("ReserveValueDto", dto);
		return "redirect:/purchase/trade/reserve/step5";
	}
	...
	public String step5Next() {
		return "redirect:/purchase/trade/reserve/step6"; // 완료 리다이렉트
	}
